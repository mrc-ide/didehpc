...
test_that("common_lib", {
  ...
  owd <- prepare_didehpc("common_lib", "mysources.R")
  on.exit(setwd(owd))

  config <- didehpc::didehpc_config(r_version = "3.2.4", use_common_lib = FALSE)
  expect_null(config$common_lib)
  config <- didehpc::didehpc_config(r_version = "3.2.4", use_common_lib = TRUE)
